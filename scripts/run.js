...
const main = async () => {
    ...
    */
    ...
    const owners = [owner, randomAccount, other];

    // WHITELIST
    let whitelist = await Contract.connect(owner).addToFactoryWhitelist([randomAccount, other]);
    ...
    const recovery = await ethers.deployContract("Recovery",[owners, 2, "RECOVERY TRUSTY"], { value: 0});
    ...
    const Trusty = await ethers.deployContract("Trusty", [owners, 2, "SingleTrusty",]/*  [anonymous.address], recoveryAddr, BLOCKLOCK, */, { value: 0 });
    ...
    await networkHelpers.mine(1).then(async () => {
        // Get the balance of a Trusty like an RPC
        const factoryBalance = await ethers.provider.getBalance(await Contract.getAddress());
        //console.log("[Factory balance]:",factoryBalance, await Contract.getAddress());
    });

    await networkHelpers.mine(1).then(async () => {
        const trustyMixBalance = await ethers.provider.getBalance(trustyMixAddr);
    ...
    await networkHelpers.mine(ABSOLUTE_TIMELOCK + 108).then(async () => {
        // Execute a tx
        ...
        const receiver = await ethers.provider.getBalance(other.address);
        ...
        console.log(new Date(parseInt(txGet2[6]) * 1000))
    });
    ...
    */
    ...
    await networkHelpers.mine(ABSOLUTE_TIMELOCK + 121).then(async () => {
        // ATTACK RECOVERY
    ...
    const recoBalance = await ethers.provider.getBalance(recoveryAddr);
    ...
    await networkHelpers.mine(ABSOLUTE_TIMELOCK + 120).then(async () => {
